...
mydict = ['Chi Thanh', 'Huu Loc', 'Linh Phan', 'Thai Hung']
...
def visualize(input, faces, fps, thickness=2):
    if faces[1] is not None:
        for idx, face in enumerate(faces[1]):
            ...
            cv.putText(input, label, (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)

            # Draw landmarks
            for i in range(5):
                cx, cy = coords[4 + i * 2], coords[5 + i * 2]
                cv.circle(input, (cx, cy), 2, (255, 0, 255), thickness)
